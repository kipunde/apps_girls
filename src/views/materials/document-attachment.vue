...
async fetchAttachments() {
  const fileType = 'document'; // For reference, can be used in API if needed
  this.loading = true;

  try {
    const res = await apiService.getModuleAttachments(); // Call API
    if (res.code === 200) {
      ...
      this.attachments = res.attachments
        .map(a => ({
          ...
          file_extension: a.file_path ? a.file_path.split('.').pop().toLowerCase() : ""
        }))
        .filter(a => {
          // Only include document files
          const allowedExtensions = ["txt", "doc", "docx", "xls", "xlsx", "pdf"];
          return allowedExtensions.includes(a.file_extension);
        });
    }
  } catch (err) {
    ...
    onFileChange(event) {
      ...
      const allowedExtensions = ["txt", "doc", "docx", "pdf", "csv", "xls", "xlsx"];
    ...
    <div class="content">
      <div class="page-header d-flex justify-content-between align-items-center mb-3">
        ...
        <button class="btn btn-added" @click="addAttachment">
          <vue-feather type="plus-circle" class="me-2" /> Add Document
        </button>
      </div>
