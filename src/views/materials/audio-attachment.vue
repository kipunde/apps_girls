...
async fetchAttachments() {
  const fileType = 'audio'; // For reference, can be used in API if needed
  this.loading = true;
  ...
  try {
    ...
    onFileChange(event) {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        Swal.fire(
          "Invalid File",
          "Only mp3, wave, ogg, m4a files are allowed.",
          "warning"
        );
    ...
    <div class="content">
      <div class="page-header d-flex justify-content-between align-items-center mb-3">
        <div class="page-title">
          <h4>Audio Attachments</h4>
          <h6>Manage materials</h6>
        </div>
        <button class="btn btn-added" @click="addAttachment">
          <vue-feather type="plus-circle" class="me-2" /> Add Audio
        </button>
      </div>
